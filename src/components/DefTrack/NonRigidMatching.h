...
class NonRigidMatching {
   public:
    ...
    void align_and_cluster(PointCloud<PointXYZRGB>::Ptr &frame, PointCloud<PointXYZ>::Ptr &mechanical_mesh_points, Matrix4f &transform_init, PolygonMesh &model, RigidTracking &rTrack, vpMbGenericTracker &tracker, vector<PointXYZ> &matched_points, vector<int> &indices, int status, int count);
    vpMbGenericTracker initialize_rigid_tracking();
    void initialize(PointCloud<PointXYZRGB>::Ptr &frame_, Matrix4f &transform_init, RigidTracking &rTrack, vpMbGenericTracker &tracker, string config_path, string cao_model_path, vpHomogeneousMatrix &cMo, int pcd_count);
    string data_folder;
